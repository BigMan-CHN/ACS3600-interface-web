...
bool CTimeCvt::string2Datetime(string str, time_t &time)
{
    try{
        char *cha = (char*)str.data();             // 将string转换成char*。
        tm tm_;                                    // 定义tm结构体。
        int year, month, day, hour, minute, second;// 定义时间的各个int临时变量。
        int res = sscanf(cha, "%d-%d-%d %d:%d:%d", &year, &month, &day, &hour, &minute, &second);// 将string存储的日期时间，转换为int临时变量。
        if(res == EOF){
            mLogError("Failed to run sscanf(%d-%d-%d %d:%d:%d),string:"<<str);
            return false;
        }
        ...
        mLogError("Failed to try string2Datetime(...),e:"<<e.what());
        return false;
    }
...
bool CTimeCvt::datetime2String(time_t time, string &str)
{
    try{
        ...
        string sTime(s);                             // 定义string变量，并将总日期时间char*变量作为构造函数的参数传入。
        str = sTime;
    }catch(exception &e){
        mLogError("Failed to try datetime2String(...),e:"<<e.what());
        return false;
    }
